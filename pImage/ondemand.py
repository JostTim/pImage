...
class OnDemandArray(np.ndarray):
    """
    This class can be called to create an object accessible like an array to generate frames on demand, in a low ram intensive mode but high disk usage mode. 
    It will slow down the processes that use the array, but make some os these processes possible that are not possible without using huge amounts of ram. (due to very large amount of data or very high number or arrays loaded in ram at the same time)
    ...
    def __new__(subtype, shape, dtype=float, buffer=None, offset=0,
                strides=None, order=None, generator_parent = None):
        
        if generator_parent is None :
            raise ValueError("A DelayedArray must be provided a generator_parent at creation to be able to get image array frames later on")
    ...
    def get_frame(self,index):
        ...
        if isinstance(index,slice):
            #raise NotImplementedError("A DelayedFramesArray cannot be iterated over with a slice. Must specify a single frame to access.")
